...
async function AboutSection() {
  ...
  return (
    <section className="flex flex-col gap-6 text-center min-h-screen mt-10" id="about">
      ...
      <article className="flex flex-col gap-10 justify-center items-center">
        {aboutData.map((card) => {
          return <AboutCard key={card.title} title={card.title} description={card.description} facts={card.facts} />;
        })}
      </article>
    </section>
  );
